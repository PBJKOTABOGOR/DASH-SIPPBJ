...
window.__moduleInit = function ({ container }) {
  ...
  on(EL.btnExportDetail, 'click', () => exportRows(getFilteredRawRows(), 'detail-ekontrak-semua-filter.csv'));
  on(EL.btnExportCurrentDetail, 'click', () => exportRows(state.selectedDetailRows, `detail-ekontrak-${slugify(state.selectedOpd || 'semua-opd')}.csv`));
