...
window.__moduleInit = function ({ container }) {
  ...
  on(EL.btnExportDetail, 'click', () => exportRows(getFilteredRawRows(), 'detail-etendering-semua-filter.csv'));
  on(EL.btnExportCurrentDetail, 'click', () => exportRows(state.selectedDetailRows, `detail-etendering-${slugify(state.selectedOpd || 'semua-opd')}.csv`));
